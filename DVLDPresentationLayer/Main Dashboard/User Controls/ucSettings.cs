...
namespace DVLDPresentationLayer.Main_Dashboard.User_Controls
{
    public class ucSettings : UserControl
    {
        ...
        private void _Build()
        {
            ...
            {
                Text = "Manage users, permissions, institutes, and system configuration.",
            ...
            {
                ("Manage Users", "Add, edit, or deactivate system user accounts.", "👤", () => { new frmListUsers().ShowDialog(); }),
                ("Manage People", "Browse the central person registry.", "👥", () => { new frmListPeople().ShowDialog(); }),
                ("Driving Institutes", "Manage driving school registrations.", "🏫", () => { new Manage_Driving_Institutes.frmListDrivingInstitutes().ShowDialog(); }),
                ("Training Batches", "Manage training batches for driving schools.", "📚", () => { new frmListTrainingBatches().ShowDialog(); }),
                ("Change Password", "Update your account password.", "🔑", () => { new frmChangePassword(Global_Classes.clsGlobal.CurrentUser.UserID).ShowDialog(); }),
            };
