        ...
        private string SaveImageToDisk(string base64String, int applicationId, string docType)
        {
            ...
            if (base64String == "existing")
            {
                try
                {
                    ...
                    string placeholderPath = System.IO.Path.Combine(folderPath, $"placeholder_{docType}.jpg");
                    if (!System.IO.File.Exists(placeholderPath))
                    {
                        System.IO.File.WriteAllBytes(placeholderPath, new byte[] { 0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46 });
                    }
